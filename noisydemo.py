...
def main(cfg):
    type = "1PR" ### to mention what you are working with 
    ...
    #print(os.path.join(os.path.dirname(__file__), 'clean_train'))
    clean_dir = os.path.dirname(__file__) + "/clean_train/" +type
    ...
    num_samples = 0
    idx_s=0
    #print(len(cleanfilenames))
    #print(cleanfilenames[1])
    for idx_s in tqdm(range(len(cleanfilenames))):
        #print("Filename",cleanfilenames[idx_s])
        num_samples=0
        ...
        #idx_s = np.random.randint(0, np.size(cleanfilenames))
            ...
            #print(cleanfilenames[idx_s])
            #print("Sampling Rate: ",fs)
            if len(clean) == 0:
                print("Silent")
                continue #silent file
...
if __name__=="__main__":
    ...
    parser.add_argument("--cfg", default = "noisy.cfg", help = "Read noisyspeech_synthesizer.cfg for all the details")
